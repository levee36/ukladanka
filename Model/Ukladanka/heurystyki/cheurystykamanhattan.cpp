...
double CHeurystykaManhattan::policzOcene(CArray2D<int> *stan)
{
    ...
    for(int t=0;t<stan->getN()*stan->getM();t++)
        if (stan->dostepLiniowy(t)>0 && (t+1)!=stan->dostepLiniowy(t)) suma_odleglosci+=odleglosc(t,stan->dostepLiniowy(t),stan->getN());
...
{
    int n = n1-n2 < 0 ? n2-n1 : n1-n2;
    int m = m1-m2 < 0 ? m2-m1 : m1-m2;
    return n+m;
}
